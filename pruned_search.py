...
class Game:
  ...
  def max(self, state):
    self._visited = 0 # reset visted nodes
    ...
    for a in actions:
      ...
      v = self.min_value(self.result(state, a), float('-inf'), float('inf'))
  ...
  def min(self, state):
    ...
    for a in actions:
      ...
      v = self.max_value(self.result(state, a), float('-inf'), float('inf'))
  ...
  def max_value(self, state, alpha, beta):
    ...
    for a in self.actions(state):
      val = max(val, self.min_value(self.result(state, a), alpha, beta))
      #check if the value is greater than beta and thus higher than its current higher bound
      if val >= beta:
        return val
      #alpha is the value that would be selected so the smaller value can be pruned
      alpha = max(alpha, val)
    return val
  #dual recursive function to find value of action
  def min_value(self, state, alpha, beta):
    ...
    #iterate through the possible action and choose the value which maximises the current value vs the returned value
    for a in self.actions(state):
      val = min(val, self.max_value(self.result(state, a), alpha, beta))
      #check if the value is less than alpha and thus can lower max's lower bound
      if val <= alpha:
        return val
      #choose the lower upper bound that min allows max to have
      beta = min(beta, val)
    return val
...
def performance_evaluation(n_max, m_max, k_max) :
  ...
  _m_evaluation(3, m_range, 3)
  _k_evaluation(3, 10, k_range)
